...
public class MinerUtils {
    ...
    public static co.rsk.bitcoinj.core.BtcBlock getBitcoinMergedMiningBlock(co.rsk.bitcoinj.core.NetworkParameters params, List<BtcTransaction> transactions) {
        ...
        long height = 1;
        return new co.rsk.bitcoinj.core.BtcBlock(params, co.rsk.bitcoinj.core.BtcBlock.getCurrentVersion(params), prevBlockHash, null, time, difficultyTarget, height, 0, transactions);
    }
        ...
        for (org.ethereum.core.Transaction tx : txs) {
            try {
                ...
                if (!(tx instanceof RemascTransaction) && tx.getGasPrice().compareTo(minGasPrice) < 0) {
                    logger.warn("Rejected tx={} because of low gas account {}, removing tx from pending state. tx.getGasPrice(): {} - minGasPrice: {}", hash, txSender, tx.getGasPrice(), minGasPrice);
