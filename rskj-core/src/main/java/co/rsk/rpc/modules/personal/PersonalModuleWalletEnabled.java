...
public class PersonalModuleWalletEnabled implements PersonalModule {
    ...
    @Override
    public String[] dumpPrivPubKeys(String address) throws Exception {
        String s = null;
        try {
            Account account = wallet.getAccount(new RskAddress(convertFromJsonHexToHex(address)));
            if (account == null) {
            ...
            keys[0] = TypeConverter.toJsonHex(Hex.toHexString(account.getEcKey().getPrivKeyBytes()));
            keys[1] = TypeConverter.toJsonHex(Hex.toHexString(account.getEcKey().getPubKey()));
