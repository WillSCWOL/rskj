...
public class BridgeSupport {
     ...
     * @param btcTxRefSerialized The raw bitcoin transaction with id=tx.vin[0].prevout.hash
     * @param heightRef The height of the bitcoin block that contains the txRef
     * @param pmtRefSerialized The raw Partial Merklee Tree that proves the txRef is included in the btc block
     * @throws BlockStoreException
     * @throws IOException
     */
    public void registerBtcTransaction(Transaction rskTx, byte[] btcTxSerialized, int height, byte[] pmtSerialized, byte[] btcTxRefSerialized, int heightRef, byte[] pmtRefSerialized) throws IOException, BlockStoreException {
        Context.propagate(btcContext);
        ...
        //TxRef Validations
        Sha256Hash btcTxRefHash = BtcTransactionFormatUtils.calculateBtcTxHash(btcTxRefSerialized);

        if (!PartialMerkleTreeFormatUtils.hasExpectedSize(pmtRefSerialized)) {
            throw new BridgeIllegalArgumentException("PartialMerkleTreeRef doesn't have expected size");
        }

        Sha256Hash merkleRootRef;
        try {
            PartialMerkleTree pmtRef = new PartialMerkleTree(bridgeConstants.getBtcParams(), pmtRefSerialized, 0);
            List<Sha256Hash> hashesInPmtRef = new ArrayList<>();
            merkleRootRef = pmtRef.getTxnHashAndMerkleRoot(hashesInPmtRef);
            if (!hashesInPmtRef.contains(btcTxRefHash)) {
                logger.warn("Supplied tx is not in the supplied partial merkle tree");
                return;
            }
        } catch (VerificationException e) {
            throw new BridgeIllegalArgumentException("PartialMerkleTree could not be parsed " + Hex.toHexString(pmtRefSerialized), e);
        }

        if (heightRef < 0) {
            String panicMessage = String.format("Height is %d but should be greater than 0", heightRef);
            logger.warn(panicMessage);
            panicProcessor.panic("btclock", panicMessage);
            return;
        }
        ...
        if (heightRef >= height) {
            logger.warn("HeightRef " + heightRef + " is not lower than Height " + height);
            panicProcessor.panic("btclock", "HeightRef " + heightRef + " is not lower than Height " + height);
            return;
        }

        // Check the the merkle root equals merkle root of btc block at specified height in the btc best chain
        // BTC blockstore is available since we've already queried the best chain height
        BtcBlock blockHeaderRef = BridgeUtils.getStoredBlockAtHeight(btcBlockStore, heightRef).getHeader();
        if (!blockHeaderRef.getMerkleRoot().equals(merkleRootRef)) {
            String panicMessage = String.format(
                    "Supplied merkle root %s does not match block's merkle root %s",
                    merkleRootRef,
        ...
        BtcTransaction btcTxRef = new BtcTransaction(bridgeConstants.getBtcParams(), btcTxRefSerialized);
        btcTxRef.verify();
        if (btcTxRef.getOutputs().isEmpty()) {
            logger.warn("TxRef has no outputs " + btcTxRef);
            panicProcessor.panic("btclock", "TxRef has no outputs " + btcTxRef);
            return;
        }
        
        // Check that the txRef is the transaction reference of the first input of tx
        if(btcTx.getInput(0).getOutpoint().getHash().compareTo(btcTxRef.getHash()) != 0) {
            logger.warn("TxRef is no the transaction reference of the first imput of Tx. TxRef: " + btcTxRef);
            panicProcessor.panic("btclock", "TxRef is no the transaction reference of the first imput of Tx. TxRef: " + btcTxRef);
            return;
        }
        ...
        if (BridgeUtils.isLockTx(btcTx, getLiveFederations(), btcContext, bridgeConstants)) {
            ...
            Script scriptPubKey = btcTxRef.getOutputs().get((int)btcTx.getInput(0).getOutpoint().getIndex()).getScriptPubKey();
            // Get the sender public key. Kcoin implementation needs the signed message to recover pubkey
           	byte[] sigBytes = scriptSig.get().getChunks().get(0).data;
        	byte[] prog = scriptPubKey.getProgram();
            byte[] connectedScript = Arrays.copyOfRange(prog, 0,  prog.length);
        	...
        	Sha256Hash hash = btcTx.hashForSignature(0, connectedScript, (byte) sigBytes[sigBytes.length - 1]);
        	            	
        	BtcECKey senderBtcKey = new BtcECKey();
        	try {
        		senderBtcKey = BtcECKey.signedToKey(hash, sigBytes, true);
        	}catch(Exception ec) {
        		logger.warn("Error recovering pubkey from signature");
        		panicProcessor.panic("btclock", "Error recovering pubkey from signature");
        		return;
        	}
            ...
            if (!lockWhitelist.isWhitelistedFor(senderBtcAddress, totalAmount, height)) {
                ...
                org.ethereum.crypto.ECKey key = org.ethereum.crypto.ECKey.fromPublicOnly(senderBtcKey.getPubKey());
    ...
    private void processSigning(BtcECKey federatorPublicKey, List<byte[]> signatures, byte[] rskTxHash, BtcTransaction btcTx) throws IOException {
        ...
        for (int i = 0; i < numInputs; i++) {
            ...
            try {
                sig = BtcECKey.ECDSASignature.decodeFromCompact(signatures.get(i));
            ...
            //Kcoin implementation doesnt necessarily send canonical sigs( we cannot canonicalize the sig because it will break the sig to pubkey algorithm)
    ...
    /**
     * Checks if the release btc tx referenced in the rskTxHash is awaiting a signature for the federator
     *
     * @param federatorPublicKey   Federator who is signing
     * @param rskTxHash            The id of the rsk tx
     */
    public Boolean isAwaitingSignByFederator(BtcECKey federatorPublicKey, byte[] rskTxHash) throws Exception {
        Context.propagate(btcContext);
    ...
    /**
     * Clears all signatures from the release btc tx referenced in the rsk tx hash
     *
     * @param executionBlockNumber The block number of the block that is currently being procesed
     * @param rskTxHash            The id of the rsk tx
     */
    public void clearSignatures(long executionBlockNumber, byte[] rskTxHash) throws Exception {
        Context.propagate(btcContext);
        ...
        for (int i = 0; i < btcTx.getInputs().size(); i++) {
            ...
            byte[] program = chunks.get(chunks.size() - 1).data;
            Script redeemScript = new Script(program);
            txIn.setScriptSig(ScriptBuilder.createP2SHMultiSigInputScript(null, redeemScript));
        }    
    }
    ...
    private boolean isInputSignedByThisFederator(BtcECKey federatorPublicKey, Sha256Hash sighash, TransactionInput input) {
        ...
        for (int j = 1; j < chunks.size() - 1; j++) {
            ...
            TransactionSignature sig2 = TransactionSignature.decodeFromSpreadcoin(chunk.data, false, false);
