...
public class Bridge extends PrecompiledContracts.PrecompiledContract {
 ...
 // Parameters:
    ...
    //KCOIN MANDATORY FOR PUBKEY RECOVERY
    // - bitcoin txref with id=tx.vin[0].prevout.hash, serialized with the bitcoin wire protocol format
    // - The bitcoin block height that contains the txref
    // - A merkle tree that shows the txref is included in that block, serialized with the bitcoin wire protocol format.
    ...
    // Returns true if the release btc tx referenced in the rsk tx hash is awaiting a signature by the federator
    // Parameters:
    // Federator public key.
    // Rsk tx hash of the tx that required the release of funds.
    public static final CallTransaction.Function IS_AWAITING_SIGN_BY_FEDERATOR = BridgeMethods.IS_AWAITING_SIGN_BY_FEDERATOR.getFunction();
    // Clears all signatures from the release btc tx referenced in the rsk tx hash
    // Parameters:
    // Rsk tx hash of the tx that required the release of funds.
    public static final CallTransaction.Function CLEAR_SIGNATURES = BridgeMethods.CLEAR_SIGNATURES.getFunction();
    ...
    public void registerBtcTransaction(Object[] args) {
        ...
        byte[] btcTxRefSerialized = (byte[]) args[3];
        int heightRef = ((BigInteger)args[4]).intValue();

        byte[] pmtRefSerialized = (byte[]) args[5];
        
        try {
            bridgeSupport.registerBtcTransaction(rskTx, btcTxSerialized, height, pmtSerialized, btcTxRefSerialized, heightRef, pmtRefSerialized);
    ...
    public void addSignature(Object[] args)
    {
        ...
        for (Object signatureObject : signaturesObjectArray) {
            ...
            try {
                BtcECKey.ECDSASignature.decodeFromCompact((byte[])signatureObject);
            } catch (Exception e) {
    ...
    public Boolean isAwaitingSignByFederator(Object[] args)
    {
        logger.trace("isAlreadySignedByFederator");
        ...
        byte[] rskTxHash = (byte[]) args[1];
        if (rskTxHash.length!=32) {
            throw new BridgeIllegalArgumentException("Invalid rsk tx hash " + Hex.toHexString(rskTxHash));
        }
                
        try {
            return bridgeSupport.isAwaitingSignByFederator(federatorPublicKey, rskTxHash);
        } catch (BridgeIllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
            logger.warn("Exception in isAlreadySignedByFederator", e);
            throw new RuntimeException("Exception in isAlreadySignedByFederator", e);
        }
        
    }
    
    public void clearSignatures(Object[] args)
    {
        logger.trace("clearSignatures");
        
        byte[] rskTxHash = (byte[]) args[0];
        if (rskTxHash.length!=32) {
            throw new BridgeIllegalArgumentException("Invalid rsk tx hash " + Hex.toHexString(rskTxHash));
        }
        
        try {
            bridgeSupport.clearSignatures(rskExecutionBlock.getNumber(), rskTxHash);
        } catch (BridgeIllegalArgumentException e) {
            throw e;
        } catch (Exception e) {
            logger.warn("Exception in clearSignatures", e);
            throw new RuntimeException("Exception in clearSignatures", e);
        }
    }
