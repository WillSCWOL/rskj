package org.ethereum.config.blockchain;

import java.math.BigInteger;

import org.ethereum.config.Constants;
import org.ethereum.core.BlockHeader;

import co.rsk.config.BridgeConstants;
import co.rsk.config.BridgeMainNetConstants;
import co.rsk.core.BlockDifficulty;

public class MainNetBeforeBridgeSyncConfig extends GenesisConfig {

	public static class MainNetBeforeConstants extends GenesisConstants {
        private static final BigInteger DIFFICULTY_BOUND_DIVISOR = BigInteger.valueOf(50);
        private static final byte CHAIN_ID = 60;
        private final BlockDifficulty minimumDifficulty = new BlockDifficulty(BigInteger.valueOf((long) 50E5 / 2));
        private final BlockDifficulty  fallbackMiningDifficulty = new BlockDifficulty(BigInteger.valueOf((long) 50E5));

        @Override
        public BridgeConstants getBridgeConstants() {
            return BridgeMainNetConstants.getInstance();
        }

        @Override
        public int getDurationLimit() {
            return 14;
        }
        
        @Override
        public BlockDifficulty getMinimumDifficulty() {
            return minimumDifficulty;
        }
        
        @Override
        public BlockDifficulty getFallbackMiningDifficulty() { 
        	return fallbackMiningDifficulty; 
        }
        
        @Override
        public BigInteger getDifficultyBoundDivisor() {
            return DIFFICULTY_BOUND_DIVISOR;
        }

        @Override
        public int getNewBlockMaxSecondsInTheFuture() {
            return 60;
        }

        @Override
        public byte getChainId() {
            return org.ethereum.config.blockchain.MainNetBeforeBridgeSyncConfig.MainNetBeforeConstants.CHAIN_ID;
        }

    }
    
    public MainNetBeforeBridgeSyncConfig() {
        super(new org.ethereum.config.blockchain.MainNetBeforeBridgeSyncConfig.MainNetBeforeConstants());
    }

    protected MainNetBeforeBridgeSyncConfig(Constants constants) {
        super(constants);
    }


    @Override
    public BlockDifficulty calcDifficulty(BlockHeader curBlock, BlockHeader parent) {
        // If more than 10 minutes, reset to original difficulty 0x00100000
        if (curBlock.getTimestamp() >= parent.getTimestamp() + 600) {
            return getConstants().getMinimumDifficulty();
        }

        return super.calcDifficulty(curBlock, parent);
    }

    @Override
    public boolean areBridgeTxsFree() {
        return true;
    }
}
